...
const AddVocab = () => {
  ...
  const classifyImage = async (source) => {
    try {
      ...
      createAPIEndpoint('translate_eng_vn')
        .fetchById(predictions[0].className) 
        .then(res => {
            setTextTranslate(res.data['reply']);
      })
      setIsLoading(false);
    } catch (error) {
      console.log(error);
    ...
    <View style={gStyle.container}>
      ...
      <React.Fragment>
        ...
        >
          ...
          <Text style={styles.sectionHeading}>Thêm từ</Text>
                ...
                {isModelReady && isLoading ? (
                  <View style={styles.btn_upload}>
                    <Text style={styles.btnText}>
                      <Ionicons style={styles.btnIcon} color={colors.white} name="image" size={40} />Load ảnh ...</Text>
                      <ActivityIndicator
                          size="small"
                        />
                  </View>
                  ...
                  <View>
                    {isModelReady ? (
                      <View style={styles.btn_upload}>
                        <Text style={styles.btnText}>
                          <Ionicons style={styles.btnIcon} color={colors.white} name="image" size={40} />Tải ảnh</Text>
                      </View>
                    ) : (
                      <View style={styles.btn_upload}>
                        <Text style={styles.btnText}>Đang load model ...</Text>
                        <ActivityIndicator
                          size="small"
                        />
                      </View>
          ...
          <View style={styles.container}>
            <View style={gStyle.flex1}>
              <Text style={styles.title_content}>Từ vựng</Text>
              <TextInput style={styles.input} value={(predictions !== null) ? predictions[0].className : "hello"}></TextInput>
            </View>
          </View>
          <View style={styles.container}>
            <View style={gStyle.flex1}>
              <Text style={styles.title_content}>Nghĩa</Text>
              <TextInput style={styles.input} value={(textTranslate !== "") ? textTranslate : "xin chào"} ></TextInput>
            </View>
          </View>
          <View style={styles.container}>
